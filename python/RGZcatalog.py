...
def RGZcatalog(survey='first'):
    """Generate the RGZ catalog database.

    Parameters
    ----------
    survey : str
        'first' or 'atlas'. Specifies what survey to generate catalog for.
    """
    ...
    #for subject in subjects.find({'zooniverse_id':'ARG0003f9l'}): #sample subject with multiple-component source
        ...
        for source in consensus.find({'zooniverse_id':subject['zooniverse_id'], ('%s_id' % survey):{'$exists':True}}):
            ...
            if process:
                ...
                #if an IR peak exists, search AllWISE/SDSS or SWIRE for counterparts
                ...
                try:
                    ...
                    #use WISE/SWIRE catalog name if available
                    if ir_pos and survey == 'atlas' and swire_match:  # need ir_pos to ensure swire_match exists; this short-circuits
                        entry.update({'rgz_name':'RGZ{}{}'.format(swire_match['designation'][5:14], swire_match['designation'][15:22])})
                    if ir_pos and survey == 'first' and wise_match:  # as above
